...
{
    public class PlaylistCacheService<T> : IPlaylistCacheService<T>
    {
        private readonly IDatabase _database;

        private const string CacheKey = "Playlist:All";
        private static readonly TimeSpan CacheDuration = TimeSpan.FromSeconds(30);

        private readonly SemaphoreSlim _semaphore = new SemaphoreSlim(1, 1);

        public PlaylistCacheService(IConnectionMultiplexer connectionMultiplexer)
        {
        ...
        public async Task<List<T>> GetAllPlaylists()
        {
        ...
        public async Task SetAllPlaylists(List<T> items)
        {
            await _semaphore.WaitAsync();
            try
            {
